...
#                 first_pass = False
            ...
            observation, reward, done, info = self.env.step(tf.squeeze(u)+tf.random.normal(shape=u.shape))
...
if __name__ == "__main__":
    split_model = split_KF_Model(model_name = 'testing_LQR_loss',initial_state_variation=[1,0.1,0.1,0.1],control=True)
    try:
        epoch = pkl.load(open(split_model.model_name+'/globalepoch.pkl','rb'))
    except FileNotFoundError:
        epoch = 100
    meta_epoch = epoch/100
    optimizer = tf.train.AdamOptimizer(1e-2*np.power(1.05,-meta_epoch))
    train(split_model,100,optimizer, view_rate=100, loss_type='look ahead',prediction_horizon=15)
